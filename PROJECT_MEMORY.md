...
- XAU demo auto execution is enabled for weekday/open gold market sessions.
- BTC and XAU each use independent `max_open_trades = 1`, so they can run one Vloc position per symbol at the same time.
